...
def getObjectsAndFrame(xml_filename):
    ...
    if frame is None:
        # Maybe it's image file?
        files = GetAllFilesListRecusive(xml_filename[: xml_filename.rindex('/')],\
        ['.png', '.PNG', '.JPG', '.jpg', '.JPEG', '.jpeg'])

        pure_name = xml_filename[xml_filename.rindex('/') + 1 : xml_filename.rindex('_o.xml')]
        
        image_file = ''
        for file in files:
            if pure_name in file:
                image_file = file
                break
    ...
    if not isinstance(frame, np.ndarray):
        print('Cant read image file', image_file)
        return
